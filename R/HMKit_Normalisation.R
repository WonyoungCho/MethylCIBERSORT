...
#BMIQ normalisation function.

HMkit.BMIQ<-function(vals,nfit=5000,plots=F){
  ...
  design.mat<-data.frame(cbind(probeInfoALL.lv$probeID,probeInfoALL.lv$Design))
  colnames(design.mat)<-c("probeID","Value")

  #Preprocessing step to render data compatible with design...

  design.mat<-design.mat[match(rownames(vals),design.mat$probeID),]
  design.v<-as.numeric(as.character(design.mat$Value))

  #Complete cases for analysis - remove infinites and that.
 
 vals<-vals[complete.cases(vals),]

  #Pad values to avoid infinities
  ...
  Matrix.obj<-matrix(data=0,nrow=nrow(vals),ncol=ncol(vals))

  #Call function
  for(i in 1:ncol(vals)){Matrix.obj[,i]<-BMIQ(vals,design.v,nfit=nfit,sampleID=i,plots=plots)$nbeta[,i]}

  #Set names
  rownames(Matrix.obj)<-rownames(vals)
  colnames(Matrix.obj)<-colnames(vals)
  return(Matrix.obj)
